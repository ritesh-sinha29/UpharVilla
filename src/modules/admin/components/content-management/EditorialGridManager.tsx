    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        {[1, 2, 3, 4, 5].map((num) => {
          ...
          return (
            <Card key={num} className="overflow-hidden group relative border-primary/5 hover:border-primary/20 transition-all">
              <div className="aspect-[3/4] bg-muted relative">
                {slot?.image ? (
                  <img src={slot.image} className="object-cover w-full h-full" alt="" />
                ) : (
      ...
      <Dialog open={isEditingSlot !== null} onOpenChange={(open) => !open && setIsEditingSlot(null)}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            <DialogTitle>Edit Slot {isEditingSlot}</DialogTitle>
            <DialogDescription className="text-xs">
                {isEditingSlot === 1 || isEditingSlot === 5 ? "Best for Portrait images." : isEditingSlot === 2 ? "Best for Wide images." : "Best for Square images."}
            </DialogDescription>
          </DialogHeader>
          <div className="grid gap-4 py-2">
            <div className="grid grid-cols-2 gap-4">
                ...
                <div className="space-y-1">
                    ...
                    <Input
                        ...
                        onChange={(e) => setSlotData({ ...slotData, altText: e.target.value })}
            ...
            <div className="space-y-1">
              ...
              <Input
                ...
                onChange={(e) => setSlotData({ ...slotData, visitLink: e.target.value })}
            ...
            <div className="space-y-1">
              ...
              >
                <input
                  ...
                  accept="image/*"
                  className="hidden"
                  onChange={(e) => e.target.files && setSlotFile(e.target.files[0])}
                />
                {slotFile ? (
                  <span className="text-[10px] font-medium truncate w-full text-center">{slotFile.name}</span>
                ) : (
          ...
          <DialogFooter>
            <Button onClick={handleUpdateSlot} disabled={isSlotUploading || (!slotFile && !slotData.image)} className="w-full h-8 text-xs">
              {isSlotUploading && <Loader2 className="mr-2 h-3 w-3 animate-spin" />}
              Update Slot
            </Button>
          </DialogFooter>
        </DialogContent>
